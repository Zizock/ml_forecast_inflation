# ==========================================================
# LASSO module for time series forecasting
# ==========================================================
...
# tune and train an LASSO model with data from train/val period,
...
# ==== build config dataclass for LASSO ====
def build_config_lasso(config_path : pathlib.Path) -> config_lasso:
...
def make_pipeline(num_X_columns : list[str], # a list of numeric feature column names
    ...
    preprocessor = ColumnTransformer(
        transformers=[
            ...
            ("cat", OneHotEncoder(handle_unknown="ignore"), date_columns),
        ]
    )

    lasso_model = Lasso(
        random_state=seed,
        max_iter= 50000, # try larger number next time?
    )
...
def tune_hyperparams_optuna(
    ...
    def objective(trial) -> float:
        params = {
            "regressor__alpha": trial.suggest_float("alpha", 1e-6, 1e2, log=True),
        }
...
def run_one_horizon(data_features : pd.DataFrame,
                    ...
                    cfg: config_lasso, # import cfg from config_lasso dataclass
                    h : int,
                    ) -> pd.DataFrame:
    ...
    print(f"h={h}: best CV RMSE={best_rmse:.6f}")
    return out
...
def run(config_path="my_config.yaml"):
    ...
    cfg = build_config_lasso(config_path)
